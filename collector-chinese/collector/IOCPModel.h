...
Purpose:
	...
	* 这个类CIOCPModel是本代码的核心类，用于说明WinSock服务器端编程模型中的
	  完成端口(IOCP)的使用方法，调用这个类实现了基本的服务器网络通信的功能。
...
#ifndef _SERVER_IOCP_MODEL_H_
#define _SERVER_IOCP_MODEL_H_
...
// 默认端口
#define DEFAULT_PORT          8000    //12345
// 默认IP地址
//#define DEFAULT_IP            _T("127.0.0.1")
#define DEFAULT_IP            _T("192.168.1.101")  //127.0.0.1 这个 转换网关是不能识别的， 当然也就连接不上了
...
typedef struct _PER_IO_CONTEXT
{
	...
	SOCKET         m_sockAccept;                               // 这个网络操作所使用的Socket  //新的工作socket， 不是工作socket
	WSABUF         m_wsaBuf;                                   // WSA类型的缓冲区，用于给重叠操作传参数的
	char           m_szBuffer[MAX_BUFFER_LEN];                 // 这个是WSABUF里具体存字符的缓冲区
	OPERATION_TYPE m_OpType;                                   // 标识网络操作的类型(对应上面的枚举)
	bool           m_busy;                                     //标识该IO操作是否处于忙碌中，true为忙绿中。主要用于发送数据的判断

	CProtocol	 m_cp;									//lgl  用于解析通讯协议, tcp重新组包
...
public:
	...
	//给客户端发送数据
	bool SendData(char* dwAddr,int nPort,LPVOID lpBuf, WORD buflen);
protected:
	...
	PER_IO_CONTEXT* _GetCliectIOContext(PER_SOCKET_CONTEXT * SendSock);

	//根据IP地址和端口号，查找客户端信息
	PER_SOCKET_CONTEXT* _GetClientSocktContext(char* dwAddr,int nPort);  

	BOOL   MySendMessage(HWND ghwnd, int wpara, char* ip, char * msg, int len);
private:
